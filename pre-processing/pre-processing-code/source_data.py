...
def source_dataset(new_filename, s3_bucket, new_s3_key):
	...
	source_dataset_url = 'https://data.cdc.gov/api/views/ks3g-spdg/rows.csv'
	...
	try:
		...
		data = response.read().decode().replace('\ufffd', '-').splitlines()
		fieldnames = data[0].replace('Pneumonia, Influenza, or COVID-19 Deaths', 'Pneumonia Influenza or COVID-19 Deaths').lower().replace(' ', '_')
